import { Response } from "express";
import { BaseController } from "../../../../app/models";
import { DecodedExpressRequest } from "../../../users/http/models/decodedRequest";
import { UpVoteArticle } from "./upVoteArticle";
import { UpVoteArticleDTO } from "./upVoteArticleDTO";
import { UpVoteArticleErrors } from "./upVoteArticleErrors";

export class UpVoteArticleController extends BaseController {
	private useCase: UpVoteArticle;

	constructor(useCase: UpVoteArticle) {
		super();
		this.useCase = useCase;
	}

	async executeImpl(req: DecodedExpressRequest, res: Response): Promise<any> {
		const { userId } = req.decoded;

		const dto: UpVoteArticleDTO = {
			userId: userId,
			slug: req.body.slug,
		};

		try {
			const result = await this.useCase.execute(dto);

			if (result.isLeft()) {
				const error = result.value;

				switch (error.constructor) {
					case UpVoteArticleErrors.AuthorNotFoundError:
					case UpVoteArticleErrors.ArticleNotFoundError:
						return this.notFound(res, error.errorValue().message);
					case UpVoteArticleErrors.AlreadyUpVotedError:
						return this.conflict(res, error.errorValue().message);
					default:
						return this.fail(res, error.errorValue().message);
				}
			} else {
				return this.ok(res);
			}
		} catch (err) {
			return this.fail(res, err as Error);
		}
	}
}
